...
class Reporteguia extends Controller {
	...
	public function dataresumido(){
		/*
				...
				*/
				...
				$sql = "SELECT  to_char(gr.fecha_traslado,'DD/MM/YYYY') nrodoc
				,gr.transporte comprobante
				,gr.ruc_transporte fechanota
				,split_part(marca_nroplaca,' ',1) fechaventa
				,split_part(marca_nroplaca,' ',2) vendedor
				,gr.chofer cliente
				,gr.lic_conducir motivo
				,SUM(dg.peso::double precision*dg.cantidad) total
				FROM almacen.guia_remision gr
				JOIN (select a.idguia_remision, cast(case when b.peso = '' then '0' else b.peso end as decimal(8,2)) peso, a.cantidad FROM almacen.detalle_guia_remision a, compra.producto b where a.idproducto =  b.idproducto and a.estado = 'A') dg on dg.idguia_remision = gr.idguia_remision
				WHERE idtransporte <> 0 {$this->condicion_resumido()}";
				$sql .= "GROUP BY  gr.fecha_traslado
				, gr.transporte
				,gr.marca_nroplaca
				, gr.ruc_transporte
				,gr.lic_conducir 
	...
	public function condicion_resumido($add_where=''){
		...
		if(!empty($_REQUEST['fechainicio'])){
			if(!empty($_REQUEST['fechafin'])){
				$where.=" AND date(fecha_traslado)='{$_REQUEST['fechainicio']}' AND date(fecha_traslado)<='{$_REQUEST['fechafin']}'";
			}else{
				$where.=" AND date(fecha_traslado)='{$_REQUEST['fechainicio']}' ";
			}
		}
	...
	public function resumido(){
		...
		if(!empty($_REQUEST['fechainicio'])){
			if(!empty($_REQUEST['fechafin']))
				$this->pdf->SetTitle(utf8_decode("REPORTE DE TRANSPORTE DE ".fecha_es($_REQUEST['fechainicio']).' A '.fecha_es($_REQUEST['fechainicio'])), 11, null, true);
			else
				$this->pdf->SetTitle(utf8_decode("REPORTE DE TRANSPORTE DE ".fecha_es($_REQUEST['fechainicio'])), 11, null, true);
		}else
			$this->pdf->SetTitle(utf8_decode("REPORTE DE TRANSPORTE "), 11, null, true);
		...
		/************************** CABECERA *****************************************/		
		$this->pdf->Cell(17,10,'FECHA',1,0,'C');
		$this->pdf->Cell(55,10,'TRANSPORTISTA',1,0,'C');
		$this->pdf->Cell(21,10,'RUC',1,0,'C');
		$this->pdf->Cell(25,10,'MARCA',1,0,'C');
		$this->pdf->Cell(25,10,'PLACA',1,0,'C'); //63
		$this->pdf->Cell(55,10,'CHOFER',1,0,'C');
		$this->pdf->Cell(21,10,'LICENCIA',1,0,'C');
		$this->pdf->Cell($whit_saldo,10,'PESO TOTAL','RBL',1,'C');
		...
		$width = array(17,55,21,25,25,55,21, $whit_saldo);
	...
	public function exportar(){
		...
		$this->insert_logoExcel($Oexcel,"REPORTE DE TRANSPORTE",true);
		...
		$filename='reportetransporte'.date("dmYhis").'.xls'; //save our workbook as this file name
