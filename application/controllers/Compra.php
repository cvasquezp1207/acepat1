...
class Compra extends Controller {
	...
	public function form($data = null) {
		...
		$query = $this->db->where("estado", "A")->order_by("idmoneda", "asc")->get("general.moneda");
		$this->combobox->addItem($query->result_array(), array('idmoneda', 'descripcion', 'abreviatura', 'simbolo'));
		...
		$this->combobox->addItem("D", "Aplicar DESPUES DEL IGV");
		$this->combobox->addItem("A", "Aplicar ANTES DEL IGV");
		if( isset($data["compra"]["tipo_descuento"]) ) {
			$this->combobox->setSelectedOption($data["compra"]["tipo_descuento"]);
		}
		$data["tipodescuento"] = $this->combobox->getObject();
	...
	public function grilla() {
		...
		$this->datatables->setColumns(array('idcompra','fecha_compra',	'comprobante','proveedor','tipoventa','moneda','subtotal','igv','descuento','total'));
		
		$this->datatables->order_by("fecha_compra", "desc"); // desc default
		
		$this->datatables->setCallback("verificarRecepcionados");
		
		$columnasName = array('ID','F.Compra','Nro.Documento','Proveedor',
	...
	public function guardar() {
		...
		if($esNuevaCompra) {
			...
			if($temp["idtipoventa"] == 2) {
				...
				}else if($this->cronograma_pago->get("cancelado")=='S'){
		...
		$tipocambio = (intval($fields["idmoneda"]) == 1) ? 1 : floatval($fields["cambio_moneda"]);
		...
		foreach($fields["deta_idproducto"] as $key=>$val) {
			...
			$this->detalle_compra->set("cantidad_um", $cantidad_unidad);
		...
		if( $fields["afecta_caja"] == 'S' && $fields["idtipoventa"] == 1 ) {
			...
			$fields["idcliente"] = $fields['idproveedor'];
	...
	public function eliminar($idcompra) {
		...
		$datos = array("idcompra"=>$idcompra, "estado"=>"I", "fecha_hora_anulacion"=>date("Y-m-d H:i:s"), 
	...
	public function detalle_body($id=0){
		$q = $this->db->query("SELECT prod.descripcion producto
								...
								JOIN compra.compra p ON p.idcompra=d.idcompra
								JOIN compra.unidad u ON u.idunidad=d.idunidad
								JOIN compra.producto prod ON prod.idproducto=d.idproducto
								WHERE d.estado = 'A' and d.idcompra=$id");
		return $q->result_array();
	}
	
	public function imprimir(){
		$this->load->library("pdf");
		$this->load_model(array( "seguridad.empresa","compra.tipo_compra","seguridad.view_usuario","compra.compra"));
		$this->empresa->find($this->get_var_session("idempresa"));
		$this->compra->find($_REQUEST['id']);
		$fechas = explode(" ",$this->compra->get("fecha_compra"));
		$fecha = '';
		
		/*$fecha = explode(" ",$this->compra->get("fecha_compra"));
		if(!empty($fechas))
			$fecha = fecha_es($fechas[0])." ".date("g:i:s a",strtotime($fechas[1]));
		*/
		$aprobado = 'NO';
		if($this->compra->get("aprobado")=='S')
			$aprobado = 'SI';
		
		$atendido = 'NO';
		if($this->compra->get("atendido")=='S')
			$atendido = 'SI';
		
		$this->view_usuario->find($this->compra->get("idusuario"));
		$this->tipo_compra->find($this->compra->get("idtipocompra"));
		
		$this->pdf->SetLogo(FCPATH."app/img/empresa/".$this->empresa->get("logo"));
		
		$this->pdf->SetTitle(utf8_decode("DETALLE DE COMPRA N° {$_REQUEST['id']}"), 11, null, true);
		...
		$this->pdf->Cell(30,6,utf8_decode('TIPO COMPRA'),0,0,'L');
		$this->pdf->Cell(2,6,utf8_decode(':'),0,0,'C');
		$this->pdf->SetFont('Arial','',9);
		$this->pdf->Cell(130,6,utf8_decode($this->tipo_compra->get("descripcion")),0,0,'L');
		$this->pdf->Ln();
		*/
		$this->pdf->SetFont('Arial','B',9);
		$this->pdf->Cell(30,6,utf8_decode('FECHA COMPRA'),0,0,'L');
		$this->pdf->Cell(2,6,utf8_decode(':'),0,0,'C');
		$this->pdf->SetFont('Arial','',9);
		$this->pdf->Cell(130,6,utf8_decode($this->compra->get("fecha_compra")),0,0,'L');
		$this->pdf->Ln();
		
		$idprov = $this->compra->get("idproveedor");
		if( ! empty($idprov)) {
			$this->load_model("compra.proveedor");
			$this->proveedor->find($this->compra->get("idproveedor"));
		...
		*/
		...
		$this->pdf->Cell(2,6,utf8_decode(':'),0,0,'C');
		$this->pdf->SetFont('Arial','',9);
		$this->pdf->Cell(130,6,utf8_decode($this->compra->get("descripcion")),0,0,'L');
		$this->pdf->Ln(10);
		*/
		$this->pdf->SetFont('Arial','B',9);
