...
class Reporterecordvendedor extends Controller {
	/**
	...
	public function dataresumido(){
		$sql = "SELECT venta_detalle_view.vendedor,
			    venta_detalle_view.producto,
			    venta_detalle_view.unidadmedida,
			    sum(venta_detalle_view.cantidad_detalle) AS cantidad
			   	FROM venta.venta_detalle_view
				WHERE estado='A'
				{$this->condicion_resumido()}
				{$this->condicion_detallado()}
				GROUP BY venta_detalle_view.producto, venta_detalle_view.unidadmedida, venta_detalle_view.vendedor
				ORDER BY vendedor";
			$query      = $this->db->query($sql);
		// echo $sql;exit;
	...
	public function resumido(){
		...
		$cabecera = array('vendedor' => array('VENDEDOR',$whit_vend)
			             ,'producto' => array('PRODUCTO',$whit_prov)
							,'unidadmedida' => array('UM',$whit_um)
							,'cantidad' => array('CANT',$whit_cant)
	...
	public function array_head(){
		...
		$whit_um		= 20;
		$whit_cant	= 20;
		$total_ancho	= $whit_compr+$whit_fecha+$whit_cli+$whit_um+$whit_vend+$whit_cant;

		$cabecera = array( 'item' => array('ITEM',$whit_item,$whit_item*100/$total_ancho,0,'L')
						    ,'producto' => array('PRODUCTO',$whit_compr,$whit_compr*100/$total_ancho,0,'L')
							,'vendedor' => array('VENDEDOR',$whit_vend,$whit_vend*100/$total_ancho,0,'L')
							,'unidadmedida' => array('UM',$whit_um,$whit_um*100/$total_ancho,0,'L')
							,'cantidad' => array('CANT',$whit_cant,$whit_cant*100/$total_ancho,0,'L')
						);
