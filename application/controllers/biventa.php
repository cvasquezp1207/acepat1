...
class Biventa extends Controller {
	...
	public function return_filas(){
		...
		$sql = "SELECT fecha
				...
				,iddetalle_caja
				,iddetalle_caja
				,(SELECT usuario FROM seguridad.view_usuario u WHERE u.idusuario=detalle_caja.idusuario) cajero
				,(SELECT descripcion FROM venta.tipopago WHERE tipopago.idtipopago=detalle_caja.idtipopago) tipopago
				FROM caja.detalle_caja 
				WHERE detalle_caja.idcaja 
				IN (SELECT idcaja FROM caja.caja WHERE estado='A' ";
		...
		$sql.=$this->filtro($fields);
		// echo $sql;exit;
		$query = $this->db->query($sql." ORDER BY doc");
	...
	public function generarPDFDET(){
		...
		$filas_detalle = array( array('doc',$ancho_doc,'L')
								...
								,array('descripcion',$ancho_desc-$reducir-17,'L')
