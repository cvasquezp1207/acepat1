...
class Reporteclaroruta extends Controller {
	...
	public function imprimir(){
		...
		'1111111' dia, 
		1 orden
		from cobranza.hoja_ruta 
		where estado = 'A' order by idcobrador, idsucursal, idcliente ";
		...
		$cabecera = array('idcliente'=> array('COD_CLI',$whit_cod)
							,'idcobrador' => array('COD_USUA',$whit_stock)
							,'idsucursal' => array('CODIGO_SUC',$whit_stock)
							,'dia' => array('DIA',$whit_precio)
							,'orden' => array('ORDEN',$whit_precio)
		...
		if(!empty($_REQUEST['fechainicio']))
			if(!empty($_REQUEST['fechafin']))
				$this->pdf->SetTitle(("REPORTE SISTEMA CLARO RUTAS".fecha_es($_REQUEST['fechainicio']).' A '.fecha_es($_REQUEST['fechafin'])), 11, null, true);
			else
				$this->pdf->SetTitle(("REPORTE SISTEMA CLARO RUTAS ".fecha_es($_REQUEST['fechainicio'])), 11, null, true);
		else
			$this->pdf->SetTitle(("REPORTE SISTEMA CLARO RUTAS "), 11, null, true);
	...
	public function exportarExcel()
    {
        ...
        $this->excel->getActiveSheet()->setCellValue('A1', 'CODIGO_CLIENTE');
        $this->excel->getActiveSheet()->setCellValue('B1', 'CODIGO_USUARIO');
        $this->excel->getActiveSheet()->setCellValue('C1', 'CODIGO_SUCURSAL');
		$this->excel->getActiveSheet()->setCellValue('D1', 'DIA');
		$this->excel->getActiveSheet()->setCellValue('E1', 'ORDEN');
        ...
        $filename='RutasClaro.xls'; //save our workbook as this file name
