...
class Jfacturacion {
	...
	public function make_file_tri($tdoc, $serie, $numero, $total) {
		//$str = "1002|TRANSFERENCIA GRATUITA";
		if($total > 0) {
			$str = "9997|EXO|VAT|".number_format($total,2,'.','')."|0|";
			//$str = "9997|EXO|VAT|0.00|0|";
		}
	...
	public function make_file_det($tipo_doc, $serie, $numero, $data) {
		...
		if( ! empty($data)) {
			foreach($data as $i=>$row) {
				...
				$linea[] = "9997"; //codTriIGV																	8
				$linea[] = number_format($row["sum_igv"],2,".",""); // monto igv x item	 mtoIgvItem				9
				$linea[] = number_format($row["valor_venta"],2,".",""); // mtoBaseIgvItem						10
				//$linea[] = "0.00"; // mtoBaseIgvItem						10
				$linea[] = "EXO"; // nomTributoIgvItem															11
				$linea[] = "VAT"; // nomTributoIgvItem															12
				$linea[] = "20"; // nomTributoIgvItem															13
				...
				$linea[] = "0.00"; // monto isc x item															36
...
*/
	...
	public function make_file_venta($cab, $det,$fixed=2) {
		...
		$linea = array();
		//$linea[] = $cab["top"]; // tipo operacion (Anexo 8 Catalogo 17, venta interna)							
		...
		//$linea[] = number_format($cab["descuento"],2,'.',''); // descuentos globales							
	...
	public function make_file_nota($cab, $det, $fixed=2) {
		...
		*/
		...
		//$linea[] = number_format($totalGra+$totalIna+$totalExo+$sumaIgv,2,'.',''); // Sumatoria de Tributos		13
