...
class Jfacturacion {
	...
	public function load_razon($idsucursal=0) {
		if($this->descripcion != NULL)
			return;
		
		$sql = "select descripcion from seguridad.empresa 
			where idempresa in (
				select idempresa from seguridad.sucursal where idsucursal=?
			)";
		$query = $this->ci->db->query($sql, array($idsucursal));
		$this->descripcion = $query->row()->descripcion;
	
	}
	
	public function load_direccion($idsucursal=0) {
		if($this->direccion != NULL)
			return;
		
		$sql = "select direccion from seguridad.empresa 
			where idempresa in (
				select idempresa from seguridad.sucursal where idsucursal=?
			)";
		$query = $this->ci->db->query($sql, array($idsucursal));
		$this->direccion = $query->row()->direccion;
	
	}
	
	public function save_file($tipo_doc, $serie, $numero, $ext, $content) {
		//$real_path = "C:\Proyectos\ACEPAT";
		//$real_path = "c:\SFS_v1.3.4.2\sunat_archivos\sfs\DATA";
		$real_path = "d:\docs";
		/*
	...
	public function get_data_venta($id, $all=FALSE) {
		...
		$sql = "select coalesce(t.codtipo_operacion, '".$this->tipo_operacion_venta."') as top, 
			...
			t.subtotal+t.igv-t.descuento as total, t.idcliente, t.descuento, t.estado, c.direccion_principal dir
			from venta.venta t
	...
	public function make_file_ley($tdoc, $serie, $numero, $total,$oferta) {
		...
		if($total > 0 ) {
			...
			$str = "L|1000|".$this->ci->numeroletra->convertir(number_format($total,2,'.',''), true)."|\r\n";
			$str .= "L|2001|BIENES TRANSFERIDOS EN LA AMAZONIA REGION SELVA PARA SER CONSUMIDOS EN LA MISMA"."|\r\n";
		...
		public function make_file_ley_not($tdoc, $serie, $numero, $total) {
		$str="";
		if($total > 0 ) {
			if( ! isset($this->ci->numeroletra))
				$this->ci->load->library('numeroletra');
	...
	public function make_file_tri_not($tdoc, $serie, $numero, $total) {
		$str="";
		if($total > 0) {
			$str = "9997|EXO|VAT|".number_format($total,2,'.','')."|0|\r\n";				
		}		
	...
	public function make_file_det($tipo_doc, $serie, $numero, $data) {
		...
		if( ! empty($data)) {
			foreach($data as $i=>$row) {
				...
				$linea = array();
				$linea[] = "I"; // porIgvItem																14
				$linea[] = $conta; // porIgvItem																14
				...
				$linea[] = number_format($row["valor_unit"],2,".",""); // mtoValorUnitario						6
				$linea[] = ""; // codigo producto sunat		 codProductoSUNAT									4 falta
				$linea[] = "0.00"; // codigo producto sunat		 codProductoSUNAT									4 falta
				...
				$linea[] = ""; // codigo producto sunat		 codProductoSUNAT									4 falta
				$linea[] = ""; // codigo producto sunat		 codProductoSUNAT									4 falta
				$linea[] = "DE ACA YA NO SIRVE"; // codigo producto sunat		 codProductoSUNAT									4 falta
	...
	public function make_file_venta($cab, $det,$fixed=2) {
		...
		$linea1[] = "V"; // tipo operacion (Anexo 8 Catalogo 17, venta interna)								1
		$linea1[] = "2.1"; // tipo operacion (Anexo 8 Catalogo 17, venta interna)								1
		$linea1[] = "2.0"; // tipo operacion (Anexo 8 Catalogo 17, venta interna)								1
		$linea1[] = ""; // tipo operacion (Anexo 8 Catalogo 17, venta interna)								1
		$content1 .= implode("|", $linea1);
		$content1 .= "\r\n";
		...
		$linea[] = "";
		$linea[] = "6"; // fecha vencimientos por defecto -											4
				
		$linea[] = $this->ruc; // RUCEMPRESA
		$linea[] = $this->descripcion; // NOMBREEMPRESA
		$linea[] = $this->descripcion; // NOMBREEMPRESA
		$linea[] = $this->direccion; // DIRECCION
		$linea[] = "221001"; // UBIGEO
		$linea[] = "SAN MARTIN"; // DEPA
		$linea[] = "TOCACHE"; // PROV
		$linea[] = "";
		$linea[] = "TOCACHE"; // CIUDAD
		$linea[] = "PE"; // PAIS
		...
		$linea[] = $cab["dir"]; // nombre cliente, razon social														8
		$linea[] = "";
		$linea[] = "";
		$linea[] = "";
		$linea[] = "";
		...
		$file = $this->save_file($cab["tdoc"], $serie, $numero, "TXT", $content);
	...
	//}	
		...
		//make_file_ley($tdoc, $serie, $numero, $total,$oferta) {
		
		$str="";
		if($importeTotal > 0 ) {
			if( ! isset($this->ci->numeroletra))
				$this->ci->load->library('numeroletra');
			$str = "L|1000|".$this->ci->numeroletra->convertir(number_format($importeTotal,2,'.',''), true)."|\r\n";
			$str .= "L|2001|BIENES TRANSFERIDOS EN LA AMAZONIA REGION SELVA PARA SER CONSUMIDOS EN LA MISMA"."|\r\n";
			
			if($totalOferta > 0 ) {
			$str .= "L|1002|TRANSFERENCIA GRATUITA DE UN BIEN Y/O SERVICIO PRESTADO GRATUITAMENTE";
			}			
		} else{				
			$str = "L|1002|TRANSFERENCIA GRATUITA DE UN BIEN Y/O SERVICIO PRESTADO GRATUITAMENTE";
		}				
		...
		$content .= $str;
		// guardamos el archivo detalle
		$this->save_file($cab["tdoc"], $serie, $numero, "TXT", $content);
	...
	public function make_file_nota($cab, $det, $fixed=2) {
		...
		// creamos archivo leyenda NC
		$this->make_file_ley_not($cab["tdoc"], $serie, $numero, $cab["total"]);
		
		// creamos archivo tributo NC
		$this->make_file_tri_not($cab["tdoc"], $serie, $numero, $importeTotal);
	...
	public function crear_files($ref, $id, $idsucursal=FALSE,$fixed = 2) {
		...
		$this->load_razon($idsucursal);
		$this->load_direccion($idsucursal);
	...
	public function enviar_files($data) {
		...
		if(file_exists($data["ruta"]."/{$file}.TXT"))
			$zip_file->addFile($data["ruta"]."/{$file}.TXT", "{$file}.TXT");
		if(file_exists($data["ruta"]."/{$file}.LEY"))
			$zip_file->addFile($data["ruta"]."/{$file}.LEY", "{$file}.LEY");
		if(file_exists($data["ruta"]."/{$file}.TRI"))
			$zip_file->addFile($data["ruta"]."/{$file}.TRI", "{$file}.TRI");
