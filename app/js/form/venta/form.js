...
$("#btn_save_venta").click(function(e) {
	...
	if(v) {
		...
		v = v && $(".deta_precio", table).required({numero:true, tipo:"float", aceptaCero:precio_venta_cero});
		if(v) {
...
$("#btn-search-preventa").on("click", function(e) {
	...
	jFrame.create({
		...
		,onSelect: function(datos) {
			...
			$("#idmodalidad").val(datos.idmodalidad);
			...
			if($("#idvendedor>option[value='"+datos.idvendedor+"']").length <= 0) {
				$("#idvendedor").append('<option value="'+datos.idvendedor+'">'+datos.vendedor+'</option>');
			}
