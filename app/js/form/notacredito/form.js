		...
		if(res.length) {
			...
			for(var i in res) {
				...
				for(var j=0; j < ser.length; j++) {
					...
					table.td('<input type="text" name="deta_importe[]" class="form-control input-xs deta_importe" value="'+imp.toFixed(2)+'" readonly>');
					...
					table.td('<input type="hidden" name="deta_idproducto[]" class="deta_idproducto" value="'+data.idproducto+'">'+
						...
						'<input type="hidden" name="deta_precio_real[]" class="deta_precio_real" value="'+data.precio+'">'+
...
$("#idtipo_notacredito").on("change", function(){
	if(Number($(this).val()) == 4) {
		$(".div-descuento").removeClass("hide");
	}else{
		$("#descuento").val("");
		$(".div-descuento").addClass("hide");
	}
});
...
$("#btn_save_notacredito").click(function(e) {
	...
	v = v && $("#numero").required();
	v = v && $("#motivo").required();
	// v = v && $("#idtipo_notacredito").required();
...
function calcularPrecios() {
	...
	$("#tbl-detalle tbody tr").each(function() {
		i = Number($(".deta_cantidad", this).val()) * Number($(".deta_precio_real", this).val());
		$(".deta_precio", this).val($(".deta_precio_real", this).val());
		$(".deta_importe", this).val(i.toFixed(2));
	});
	
	calcularTotales();
	
	if($.isNumeric($("#descuento").val()) && $.isNumeric($("#total").val())) {
		var nuevototal = Number($("#descuento").val());
		var total = Number($("#total").val());
		if(nuevototal > total)
			return;
		if($("#tbl-detalle tbody tr.item-select").length <= 0)
			return;
		
		var s = 1 - ((total - nuevototal) / total);
		
		$("#tbl-detalle tbody tr.item-select").each(function() {
			p = Number($(".deta_precio", this).val()) * s;
			i = Number($(".deta_cantidad", this).val()) * p;
			$(".deta_precio", this).val(p.toFixed(2));
			$(".deta_importe", this).val(i.toFixed(2));
		});
