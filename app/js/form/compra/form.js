...
function addDetalle(data) {
	...
	table.td('<select name="deta_idunidad[]" class="form-control input-xs deta_idunidad numerillo" data-toggle="tooltip" title=""></select>');
	table.td('<input type="text" name="deta_cantidad[]" class="form-control input-xs deta_cantidad numerillo" value="'+data.cantidad+'" autocomplete="off">');
	table.td('<input type="text" name="deta_precio[]" class="form-control input-xs deta_precio numerillo" value="'+data.precio_compra+'" autocomplete="off">');
	table.td('<input type="text" name="deta_precio_igv[]" class="form-control input-xs deta_precio_igv numerillo" value="'+data.precio_compra+'" autocomplete="off">');
	table.td('<input type="text" name="deta_importe[]" class="form-control input-xs font-bold deta_importe numerillo" autocomplete="off">');
	table.td('<input type="text" name="deta_importe_igv[]" class="form-control input-xs font-bold deta_importe_igv numerillo" autocomplete="off">');
	table.td('<input type="text" name="deta_igv[]" class="form-control input-xs text-success deta_igv" readonly>');
	table.td('<input type="text" name="deta_descuento[]" class="form-control input-xs text-success deta_descuento numerillo" readonly>');
	table.td('<input type="text" name="deta_flete[]" class="form-control input-xs text-success deta_flete numerillo" readonly>');
	table.td('<input type="text" name="deta_costo[]" class="form-control input-xs text-success font-bold deta_costo numerillo" value="'+data.precio_compra+'" readonly>');
...
function descontarAntes() {
	return ($("#tipo_descuento").val() == "A");
}
...
function calcularIgvAntes(tr) {
	var igv = 0, descuento, precio;
	if(tr) {
		if($.isNumeric($("input.deta_precio", tr).val())) {
			descuento = $.isNumeric($("input.deta_descuento", tr).val()) ? Number($("input.deta_descuento", tr).val()) : 0;
			igv = 0;
			if($("#valor_igv").is(":checked")) {
				precio = Number($("input.deta_precio", tr).val()) - descuento;
				igv = precio * parseFloat($("#valor_igv").val())/100;
			}
	...
	$("#tbl-detalle tbody tr").each(function() {
		if($.isNumeric($("input.deta_precio", this).val())) {
			igv = 0;
			descuento = $.isNumeric($("input.deta_descuento", this).val()) ? Number($("input.deta_descuento", this).val()) : 0;
			if($("#valor_igv").is(":checked")) {
				precio = Number($("input.deta_precio", this).val()) - descuento;
				igv = precio * parseFloat($("#valor_igv").val())/100;
			}
...
function calcularIgvDespues(tr) {
	var igv = 0;
	if(tr) {
		if($.isNumeric($("input.deta_precio", tr).val())) {
			igv = 0;
...
function calcularIgv(tr) {
	if(descontarAntes())
		...
		calcularIgvDespues(tr);
}

function calcularPrecioIgvAntes(tr) {
	var igv, precio, descuento;
	if(tr) {
		if($.isNumeric($("input.deta_precio", tr).val())) {
			descuento = $.isNumeric($("input.deta_descuento", tr).val()) ? Number($("input.deta_descuento", tr).val()) : 0;
			...
			precio = parseFloat($("input.deta_precio", tr).val()) - descuento + igv;
	...
	$("#tbl-detalle tbody tr").each(function() {
		if($.isNumeric($("input.deta_precio", this).val())) {
			...
			precio = parseFloat($("input.deta_precio", this).val()) - descuento + igv;
...
function calcularPrecioIgvDespues(tr) {
	var igv, precio;
	if(tr) {
		if($.isNumeric($("input.deta_precio", tr).val())) {
			igv = parseFloat($("input.deta_igv", tr).val());
...
function calcularPrecioIgv(tr) {
	if(descontarAntes())
		calcularPrecioIgvAntes(tr);
	else
		calcularPrecioIgvDespues(tr);
}

function calcularIgvCompraAntes() {
	if( $.isNumeric($("#subtotal").val()) ) {
		if($("#valor_igv").is(":checked")) {
			var descuento = $.isNumeric($("#descuento").val()) ? Number($("#descuento").val()) : 0;
			var igv = (Number($("#subtotal").val()) - descuento) * parseFloat($("#valor_igv").val())/100;
...
function calcularIgvCompraDespues() {
	if( $.isNumeric($("#subtotal").val()) ) {
		if($("#valor_igv").is(":checked")) {
...
function calcularIgvCompra() {
	if(descontarAntes())
		calcularIgvCompraAntes();
	else
		calcularIgvCompraDespues();
}
...
function calcularFlete() {
	if($.isNumeric($("#total").val()) && $.isNumeric($("#flete").val())) {
		...
		if(totalFlete > 0) {
			var factorFlete = totalFlete / Number($("#total").val());
			
			$("#tbl-detalle tbody tr").each(function() {
				fleteitem = Number($(".deta_precio", this).val()) + Number($(".deta_igv", this).val()) - Number($(".deta_descuento", this).val());
				fleteitem *= factorFlete;
				$("input.deta_flete", this).val(fleteitem.toFixed(_fixed_compra));
			});
			return;
		}
...
function calcularDescuentoAntes() {
	if($.isNumeric($("#subtotal").val()) && $.isNumeric($("#descuento").val())) {
		var descuento = Number($("#descuento").val()),
			subtotal = Number($("#subtotal").val()), descitem;
		
		if(subtotal > 0) {
			var factorDescuento = descuento / subtotal;
			
			$("#tbl-detalle tbody tr").each(function() {
				descitem = 0;
				if($.isNumeric($(".deta_precio", this).val())) {
					descitem = Number($(".deta_precio", this).val()) * factorDescuento;
				}
				$(".deta_descuento", this).val(descitem.toFixed(_fixed_compra));
			});
		}
...
function calcularDescuentoDespues() {
	if($.isNumeric($("#total").val()) && $.isNumeric($("#descuento").val())) {
		var descuento = parseFloat($("#descuento").val()), cantidad = 0, descitem;
		
		if(descuento > 0) {
			var factorDescuento = descuento / (Number($("#total").val()) + descuento);
			
			$("#tbl-detalle tbody tr").each(function() {
				if($.isNumeric($(".deta_cantidad", this).val()) && Number($(".deta_cantidad", this).val()) > 0) {
					descitem = Number($(".deta_importe_igv", this).val()) * factorDescuento / Number($(".deta_cantidad", this).val());
				}
				else {
					descitem = 0;
				}
...
function calcularDescuento() {
	if(descontarAntes())
		calcularDescuentoAntes();
	else
		calcularDescuentoDespues();
}
	...
	if( $("#tbl-detalle tbody tr").length ) {
		...
		$("#tbl-detalle tbody tr").each(function() {
			if($.isNumeric($("input.deta_precio", this).val())) {
				...
				if($.isNumeric($("input.deta_descuento", this).val()) && Number($("input.deta_descuento", this).val()) > 0)
					t -= Number($("input.deta_descuento", this).val());
				if($.isNumeric($("input.deta_igv", this).val()))
					t += parseFloat($("input.deta_igv", this).val());
				if($.isNumeric($("input.deta_flete", this).val()) && Number($("input.deta_flete", this).val()) > 0)
					t += Number($("input.deta_flete", this).val());
...
$("#idmoneda").on("change", function() {
	$(".abrev-moneda").text($("option:selected", this).data("simbolo"));
	$(".abrev-moneda").data("abreviatura", $("option:selected", this).data("abreviatura"));
	if($.isNumeric($(this).val())) {
		var tipoCambio = Number($("#cambio_moneda").val());
		if(tipoCambio <= 0 || ($(this).val() != "1" && tipoCambio <= 1)) {
	...
	jFrame.create({
		...
		,onSelect: function(datos) {
			...
			if(datos.idproveedor) {
				$("#proveedor_idproveedor").val(datos.idproveedor);
				$("#proveedor_descripcion").val(datos.proveedor);
			}
...
if(_es_nuevo_compra_) {
	...
	$(".abrev-moneda").text($("#idmoneda>option:selected").data("simbolo"));
	$(".abrev-moneda").data("abreviatura", $("#idmoneda>option:selected").data("abreviatura"));
	llenarDetalleCompra();
}
...
function getMontoConvertido(monedaFactura, monedaFlete, tipoCambio, flete) {
	if(monedaFlete == undefined && tipoCambio == undefined)
		return Number(flete);
...
$("#btn-convertir").on("click", function(e) {
	e.preventDefault();
	if($("#tbl-detalle tbody tr").length) {
		if($(':radio[name="conversion"]:checked').length) {
			var op = $(':radio[name="conversion"]:checked').val();
			if($(this).data("operacion") == op) {
				var l1 = (op == "M") ? "multiplicacion" : "division";
				var l2 = (op == "M") ? "division" : "multiplicacion";
				ventana.alert({titulo:"", mensaje:"<strong>Nota</strong>: Ya se ha realizado la "+l1+". Realice ahora la "+l2});
				return;
			}
			
			var tc = $(".abrev-moneda").data("abreviatura");
			if(op == "M" && tc == "PEN") {
				ventana.alert({titulo:"", mensaje:"La multiplicacion es para convertir a Soles (PEN). "+
					"La compra actualmente se encuentra en Soles."});
				return;
			}
			
			if(op == "D") {
				$("#idmoneda").val($("#idmoneda>option:not([data-abreviatura='PEN']):first").prop("value"));
			}
			else {
				$("#idmoneda").val($("#idmoneda>option[data-abreviatura='PEN']").prop("value"));
			}
			$("#idmoneda").trigger("change");
			
			$(this).data("operacion", op);
			var cambio = Number($("#cambio_moneda").val());
			if(op == "D" && cambio == 0) {
				cambio = 1;
			}
			
			var inputs = ["deta_precio", "deta_precio_igv", "deta_importe", "deta_importe_igv", "deta_igv", "deta_flete", 
				"deta_descuento", "deta_costo"], subtotal = 0, igv = 0, flete = 0, descuento = 0, cant, temp, i;
			
			var fnConvertir = function(v) {
				if(op == "D")
					return Number(v) / cambio;
				return Number(v) * cambio;
			};
			
			var fnAddTotal = function(l, c, v) {
				if(l == "deta_precio")
					subtotal += c * v;
				else if(l == "deta_igv")
					igv += c * v;
				else if(l == "deta_flete")
					flete += c * v;
				else if(l == "deta_descuento")
					descuento += c * v;
			};
			
			$("#tbl-detalle tbody tr").each(function() {
				cant = Number($(".deta_cantidad", this).val());
				for(i in inputs) {
					temp = fnConvertir($("."+inputs[i], this).val());
					$("."+inputs[i], this).val(temp.toFixed(_fixed_compra));
					fnAddTotal(inputs[i], cant, temp)
				}
			});
			...
			if(op == "D")
				...
				subtotal = subtotal / cambio
			$("#equivalencia").val(subtotal.toFixed(2));
		}
	}
});