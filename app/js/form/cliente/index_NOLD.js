...
function OpenModalLinea(id){
	...
	ajax.post({
		...
		if(data.cliente.linea_credito=='S'){
			...
			porcen = parseFloat(data.cliente.limite_credito)*.2);
			$("#monto").val(porcen.toFixed(2));
	...
	ajax.post({
		...
		// $('#linea_credito').trigger("click");
			...
			if(data.cliente){
				...
				consumido = parseFloat(data.cliente.limite_credito) - parseFloat(data.saldo.toFixed(2));
