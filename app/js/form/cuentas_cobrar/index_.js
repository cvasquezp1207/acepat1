...
function cargarDatosVenta(idventa) {
	ajax.post({url: _base_url+"venta/get_all/"+idventa, dataType: 'json'}, function(data) {
		...
		$("#idcobrador").empty();
		if(data.cobradores.length){
			html = '<option value="">Seleccione..</option>';
			$(data.cobradores).each(function(x,y){
				html+="<option value='"+y['idcobrador']+"'>"+y["cobrador"]+"</option>";
			});
			$("#idcobrador").html(html);
		}else{
			$("#idcobrador").html("<option value='0'>Cobrador no asignado</option>");
		}
	});
}
		...
		if($("#table-letras tbody tr").length) {
			...
			if($("#total_pagar").required({numero:true, tipo:"float"})) {
				...
				if(_forma_pago_ == "L") {
					...
					//e3 = $("#table-letras tbody tr.success").length;
				}
				else {
					e1 = parseFloat($("#monto_pagar").val());
					e2 = parseFloat($("#total_pagar").val());
					e3 = parseFloat($("#total_descuento").val());
				}
				if(e1 != e2) {
					...
					if(e3 >= 1) {
					ventana.alert({titulo: "Advertencia", mensaje: "CANCELAR TODA LA DEUDA"
						+" de pago, por favor cancele toda la deuda"});
					return;
				}
				...
				if(deuda >= pagar) {
					if($.trim($("#idcobrador").val())==''){
						ventana.confirm({
							titulo:"Confirmar"
							,mensaje:"No ha seleccionado un cobrador para el pago, desea continuar?"
...
// nota de credito
$("#btn_search_notacredito").click(function(e) {
	e.preventDefault();
	if($("#nro_credito").required()) {
		// modal para los recibos de ingreso
		jFrame.create({
			title: "Buscar notas de credito"
			,controller: "notacredito"
			,method: "grilla_popup"
			,msg: ""
			,widthclass: "modal-lg"
			,data: "idventa="+$("#credito_idventa").val()+"&idmoneda="+$("#idmoneda").val()
			,onSelect: function(datos) {
				$("#tipo_recibo").val("NC");
				$("#numero_recibo").val(datos.nrodoc);
				$("#fecha_recibo").val(fecha_es(datos.fecha));
				$("#monto_recibo").val(datos.monto);
				$("#idrecibo").val(datos.idnotacredito);
			}
		});
		...
		if($("#table-letras tbody tr").length) {
			if($("#total_pagar").required({numero:true, tipo:"float"})) {
				...
				if(deuda >= pagar) {
					...
					ventana.alert({titulo:"", mensaje:"El monto del recibo es mayor a la deuda. &iquest;Que ha ocurrido?"});
				}
			}
			else {
...
function save_amortizacion(){
	...
	pay.ok(function(datos) {
		var str = $("#form-pago").serialize();
			...
			str += "&"+datos;
			str += "&venta_comprobante="+$("#venta_comprobante").val();
		...
		ajax.post({url: _base_url+"cuentas_cobrar/amortizar", data: str}, function(data) {
			ventana.alert({titulo:"", mensaje:"Pago realizado correctamente"}, function() {
				...
				$("#numero_recibo,#fecha_recibo,#monto_recibo,#idrecibo").val("");
			});
		});
	});
	pay.show();
	...
	ajax.post({url: _base_url+"cuentas_cobrar/get_amortizaciones/"+idcredito, dataType: 'json'}, function(res) {
		...
		if(res.array.length) {
			...
			for(var i in res.array) {
				...
				if(data.tipo_recibo == last.tipo_recibo && data.id_ri == last.idrecibo_ingreso && data.id_nc == last.idnotacredito) {
					rec += '<span class="pull-right">'+
						'<button class="btn btn-white btn-xs btn-del-amortizacion" data-toggle="tooltip" data-recibo="'+
						data.recibo+'" data-idri="'+data.id_ri+'" data-idnc="'+data.id_nc+'" data-tipo="'+
						data.tipo_recibo+'" data-idsucursal="'+data.idsucursal+'" title="Eliminar recibo">'+
